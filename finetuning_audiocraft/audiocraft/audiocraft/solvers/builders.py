...
from .. import adversarial, losses, metrics, optim
...
def get_audio_datasets(cfg: omegaconf.DictConfig,
    ...
    for split, path in cfg.datasource.items():
        ...
        if dataset_type == DatasetType.MUSIC:
            dataset = finetuning2.audiocraft.audiocraft.data.music_dataset.MusicDataset.from_meta(path, **kwargs)
        elif dataset_type == DatasetType.SOUND:
            dataset = finetuning2.audiocraft.audiocraft.data.sound_dataset.SoundDataset.from_meta(path, **kwargs)
        elif dataset_type == DatasetType.AUDIO:
            dataset = finetuning2.audiocraft.audiocraft.data.info_audio_dataset.InfoAudioDataset.from_meta(path, return_info=return_info, **kwargs)
        elif dataset_type == DatasetType.JASCO:
            dataset = finetuning2.audiocraft.audiocraft.data.jasco_dataset.JascoDataset.from_meta(path, return_info=return_info, **kwargs)
        else:
