...
class MusicGenSolver(base.StandardSolver):
    ...
    def build_model(self) -> None:
        ...
        self.model: tp.Union[
            models.LMModel, models.FlowMatchingModel] = finetuning2.audiocraft.audiocraft.models.builders.get_lm_model(
    ...
    #     # 🔥 Patch 1: Remove chroma projection weights from model state
    #     model_state = state.get("model", {})
    #     keys_to_remove = [k for k in model_state if "conditioners.self_wav.output_proj" in k]
    #     for k in keys_to_remove:
    #         print(f"[load_state_dict] Skipping incompatible chroma weight: {k}")
    #         del model_state[k]

    #     # 🔥 Patch 2: Remove chroma weights from Flashy best state tracking
    #     if "best_state" in state:
    #         best_state = state["best_state"]
    #         for scope in best_state:
    #             if "conditioners.self_wav.output_proj" in best_state[scope]:
    #                 keys = [k for k in best_state[scope] if "conditioners.self_wav.output_proj" in k]
    #                 for k in keys:
    #                     print(f"[load_state_dict] Removing best_state incompatible key: {k}")
    #                     del best_state[scope][k]

    #     super().load_state_dict(state)

    def load_from_pretrained(self, name: str):
        # TODO: support native HF versions of MusicGen.
        lm_pkg = finetuning2.audiocraft.audiocraft.models.loaders.load_lm_model_ckpt(name)
        state: dict = {
            'best_state': {
    ...
    def generate_audio(self) -> dict:
        ...
        print(f"dataloaders: {self.dataloaders.keys()}")
