...
class MusicGen(BaseGenModel):
    ...
    def generate_with_wav2vec(self, descriptions: tp.List[str], wav2vec_wavs: MelodyType,
                          sample_rate: int, progress: bool = False,
                          return_tokens: bool = False) -> tp.Union[torch.Tensor, tp.Tuple[torch.Tensor, torch.Tensor]]:
        """
        Generate samples conditioned on text and wav2vec2-based audio features.

        Args:
            descriptions (list of str): Text descriptions for conditioning.
            wav2vec_wavs (Tensor or list of Tensors): Audio waveforms [C, T] or list of such.
            sample_rate (int): Sample rate of input audio.
            progress (bool): Whether to show generation progress.
            return_tokens (bool): Whether to return discrete tokens alongside audio.

        Returns:
            Tensor of audio [B, C, T] or (audio, tokens)
        """
        if isinstance(wav2vec_wavs, torch.Tensor):
            if wav2vec_wavs.dim() == 2:
                wav2vec_wavs = wav2vec_wavs[None]
            if wav2vec_wavs.dim() != 3:
                raise ValueError("Wav2vec wavs must be [B, C, T]")
            wav2vec_wavs = list(wav2vec_wavs)

        wav2vec_wavs = [
            convert_audio(wav, sample_rate, self.sample_rate, self.audio_channels)
            if wav is not None else None
            for wav in wav2vec_wavs
        ]

        attributes = []
        for desc, wav in zip(descriptions, wav2vec_wavs):
            if wav is None:
                attr = ConditioningAttributes(
                    text={'description': desc},
                    wav={'self_wav2vec': WavCondition(
                        wav=torch.zeros((1, 1, 1), device=self.device),
                        length=torch.tensor([0], device=self.device),
