...
class CLAP2AudioMAE(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx=None, cond_dict=None):
        ...
        audio_embed = cond_dict["film_clap_cond1"]        # [B, 1, 512]
        text_embed = cond_dict["film_clap_cond2"]         # [B, 1, 512]
        combined_embed = torch.cat([audio_embed, text_embed], dim=-1)  # [B, 1, 1024]

        input_embeds = combined_embed
        target_embeds = cond_dict["crossattn_audiomae_pooled"][0]  # keep if used
    ...
    def generate(self, batch, cond_dict=None, no_grad=False):
        if cond_dict is None:
            cond_dict = self.get_input(batch)
        audio_embed = cond_dict["film_clap_cond1"]        # shape: [B, 1, 512]
        text_embed = cond_dict["film_clap_cond2"]         # shape: [B, 1, 512]

        combined_embed = torch.cat([audio_embed, text_embed], dim=-1)  # shape: [B, 1, 1024]
        input_embeds = combined_embed
    ...
    def validation_step(self, batch, batch_idx):
        ...
        input_embeds = combined_embed
        target_embeds = cond_dict["crossattn_audiomae_pooled"][0]  # Or ignore if not used
