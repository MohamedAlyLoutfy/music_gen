...
class AudioDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Check if we are in training mode (target exists)
        is_training = "target" in datum

        if is_training:
            # Training mode: conditioning input and target output
            _, _, cond_waveform, _ = self.read_audio_file(datum["wav"])
            log_mel_spec, stft, target_waveform, random_start = self.read_audio_file(datum["target"])
            fname = datum["target"]
            label_indices = torch.zeros(self.label_num, dtype=torch.float32)
        else:
            # Inference mode: only conditioning input, no target
            log_mel_spec, stft, cond_waveform, random_start = self.read_audio_file(datum["wav"])
            fname = datum["wav"]
            target_waveform = cond_waveform  # placeholder
            label_indices = torch.zeros(self.label_num, dtype=torch.float32)

        text = self.get_sample_text_caption(datum, None, label_indices)

        data = {
            "text": text,
            "fname": self.text_to_filename(text) if not fname else fname,
            "label_vector": label_indices,
            "audio": torch.tensor(cond_waveform).float(),
            "waveform": torch.tensor(target_waveform).float(),  # 👈 only needed during training
            "stft": stft.float() if stft is not None else "",
            "log_mel_spec": log_mel_spec.float() if log_mel_spec is not None else "",
        ...
        for add_on in self.add_ons:
            data.update(add_on(self.config, data, datum))

        if data["text"] is None:
            print("Warning: The model returned None for key text", fname)
            data["text"] = ""
    ...
    def feature_extraction(self, index):
        ...
        while True:
            try:
                ...
                # Conditioning audio — the input to CLAP
                _, _, cond_waveform, _ = self.read_audio_file(datum["wav"])

                # Target audio — the thing to generate
                log_mel_spec, stft, target_waveform, random_start = self.read_audio_file(datum["target"])
        ...
        return (
            datum["target"],        # fname
            cond_waveform,          # used as "audio"
            target_waveform,        # used as "waveform"
            stft,
            log_mel_spec,
            label_indices,
            (datum, mix_datum),
